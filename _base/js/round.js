...
$(function(){
		...
		if (racesCanvas.getContext) {
			...
			// begin canvas code
 		    ...
 		    for (i=1; i <= playerCount; i++) {
                 ...
                 hue = parseInt(hue);
                 ctx.fillStyle = $('#graph .player').eq(i-1).find('.round').css('background-color');
                 // debug(ctx.fillStyle);
    ...
    $('#animate_round:enabled').click(function(){
        ...
        $('#graph .player .round').animate({ width: 0 }, 'normal', 'swing', function(){
            if( !ajaxing ) {
        		animateRace();
            }
            ajaxing = true;
        ...
        function movePlayers() {
            $('#graph .player').each(function(i){
				...
				var roundMeter = $(this).children('.round');
